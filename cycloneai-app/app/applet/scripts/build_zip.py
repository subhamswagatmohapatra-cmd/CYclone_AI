...
ignore_dirs = {"node_modules", "dist", ".git", ".next", "__pycache__", "cycloneai-project", "scripts"}
ignore_files = {"cycloneai-complete-project.zip", "cycloneai-complete-project.tar.gz", "package_zip.py", "build_zip.py"}
...
with zipfile.ZipFile(out_zip, "w", zipfile.ZIP_DEFLATED) as zf:
    ...
    root_entry = zipfile.ZipInfo("cycloneai-app/", now_time)
    root_entry.create_system = 0  # Windows FAT
    root_entry.external_attr = 0x10  # Directory attribute
    zf.writestr(root_entry, "")
...
# Also create clean tar.gz containing the folder
with tarfile.open(out_tar, "w:gz") as tar:
    # Add files from the zip or directly
    for dirpath, dirnames, filenames in os.walk(root_dir):
        dirnames[:] = [d for d in dirnames if d not in ignore_dirs and not d.startswith(".git") and d != "public"]
        rel_dir = os.path.relpath(dirpath, root_dir)
        target_dir = "cycloneai-app" if rel_dir == "." else os.path.join("cycloneai-app", rel_dir)
        for filename in filenames:
            if filename in ignore_files or filename.endswith(".zip") or filename.endswith(".tar.gz"):
                continue
            src_path = os.path.join(dirpath, filename)
            arc_name = os.path.join(target_dir, filename)
            tar.add(src_path, arcname=arc_name)

print("Built clean TAR package:", out_tar)
